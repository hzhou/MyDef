...
sub getfieldsize {
    ...
    my $size=$ff->{size};
    if(!$size){
        if($type eq "year"){$size=4;};
        elsif($type eq "date"){$size=10; if($ff->{optional}){$valign="valign=\"middle\"";}};
        elsif($type eq "money"){$size=10;};
        elsif($type eq "zip"){$size=5;};
        elsif($type eq "int"){$size=5;};
        elsif($type eq "phone"){$size=15;};
        elsif($type eq "email"){$size=25;};
        elsif($type eq "usdollar"){$size=12;$prefix='\$';};
        else{$size=50;};
    }
    return $size;
}
...
sub parsecode {
    ...
    if($l=~/^\s*CSS: (.*)\s*\{(.*)\}/){
    ...
    elsif($mode ne 'sub' and $l!~/^\s*\$(log|call|map|include|tag|script|style|p|h[1-5]|center|pre|div|span|b|ol|ul|li|table|tr|td|th|fieldset|jssrc|divbox|if|ifz|ifnz|elif|elsif|elseif|else|foreach|for|while|function)\b/){
        if($mode eq 'php'){
            $l=terminate_php($l);
        }
        ...
        if($func eq 'php'){
        ...
        elsif($func =~ /^(tag|div|span|center|ol|ul|li|table|tr|td|th|b|script|style|p|h[1-5]|center|pre)$/){
            my $t;
            ...
            foreach my $tt (@tt_list){
                ...
                elsif($tt=~/^(top|bottom|middle)$/){
                    $t.=" valign=\"$1\"";
                }
                elsif($tt=~/^(left|right|center)$/){
                    $t.=" align=\"$1\"";
                }
                elsif($tt=~/^#([a-zA-Z_0-9\$]+)$/){
                    $t.=" id=\"$1\"";
